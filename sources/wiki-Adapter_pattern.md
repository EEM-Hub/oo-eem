...
|  | This articlemay containexcessiveorirrelevantexamples.Please helpimprove itby removingless pertinent examplesandelaboratingon existing ones.(January 2011)(Learn how and when to remove this message) |
| --- | --- |

 

In [software engineering](./Software_engineering), the **adapter pattern** is a [software design pattern](./Software_design_pattern) (also known as [wrapper](./Wrapper_function), an alternative naming shared with the [decorator pattern](./Decorator_pattern)) that allows the [interface](./Interface_(computer_science)) of an existing [class](./Class_(computer_science)) to be used as another interface.[[1]](./Adapter_pattern#cite_note-HeadFirst-1): 244  It is often used to make existing classes work with others without modifying their [source code](./Source_code).

 

An example is an adapter that converts the interface of a [Document Object Model](./Document_Object_Model) of an [XML](./XML) document into a tree structure that can be displayed.
...
The adapter[[2]](./Adapter_pattern#cite_note-GoF-2) design pattern is one of the twenty-three well-known [Gang of Four](./Gang_of_Four_(software)) design patterns that describe how to solve recurring design problems to design flexible and reusable object-oriented software, that is, objects that are easier to implement, change, test, and reuse.

 

The adapter design pattern solves problems like:[[3]](./Adapter_pattern#cite_note-Adapter_Design_Pattern-3)
...
An adapter can be used when the wrapper must respect a particular interface and must support [polymorphic](./Polymorphism_(computer_science)) behavior. Alternatively, a [decorator pattern](./Decorator_pattern) makes it possible to add or alter behavior of an interface at run-time, and a [facade pattern](./Facade_pattern) is used when an easier or simpler interface to an underlying object is desired.[[1]](./Adapter_pattern#cite_note-HeadFirst-1): 243, 252, 258, 260 
...
### UML class diagram
 ...
 [![](//upload.wikimedia.org/wikipedia/commons/e/e5/W3sDesign_Adapter_Design_Pattern_UML.jpg)](./File:W3sDesign_Adapter_Design_Pattern_UML.jpg)A sample UML class diagram for the adapter design pattern[[4]](./Adapter_pattern#cite_note-Structure-4) 

In the above [UML](./Unified_Modeling_Language) [class diagram](./Class_diagram), the `client` class that requires a `target` interface cannot reuse the `adaptee` class directly because its interface doesn't conform to the `target` interface.
...
In this adapter pattern, the adapter contains an instance of the class it wraps. In this situation, the adapter makes calls to the instance of the wrapped [object](./Object_(computing)).

 [![](//upload.wikimedia.org/wikipedia/commons/thumb/d/d7/ObjectAdapter.png/330px-ObjectAdapter.png)](./File:ObjectAdapter.png)The object adapter pattern expressed in [UML](./Unified_Modeling_Language) [![](//upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Adapter%28Object%29_pattern_in_LePUS3.png/500px-Adapter%28Object%29_pattern_in_LePUS3.png)](./File:Adapter(Object)_pattern_in_LePUS3.png)The object adapter pattern expressed in [LePUS3](./Lepus3?action=edit&redlink=1) 

### Class adapter pattern

 

This adapter pattern uses multiple [polymorphic interfaces](./Subtyping) implementing or inheriting both the interface that is expected and the interface that is pre-existing. It is typical for the expected interface to be created as a pure [interface](./Interface_(Java)) class, especially in [languages](./Programming_language) such as [Java](./Java_(programming_language)) (before JDK 1.8) that do not support [multiple inheritance](./Multiple_inheritance) of classes.[[1]](./Adapter_pattern#cite_note-HeadFirst-1): 244 

 [![](//upload.wikimedia.org/wikipedia/commons/thumb/3/35/ClassAdapter.png/330px-ClassAdapter.png)](./File:ClassAdapter.png)The class adapter pattern expressed in [UML](./Unified_Modeling_Language) [![](//upload.wikimedia.org/wikipedia/commons/thumb/b/b3/Adapter%28Class%29_pattern_in_LePUS3.png/500px-Adapter%28Class%29_pattern_in_LePUS3.png)](./File:Adapter(Class)_pattern_in_LePUS3.png)The class adapter pattern expressed in [LePUS3](./Lepus3?action=edit&redlink=1) 
...
and perhaps create the correctly "formatting" object at runtime by means of the [factory pattern](./Factory_pattern).
...
- [Delegation](./Delegation_(programming)), strongly relevant to the object adapter pattern.
- [Dependency inversion principle](./Dependency_inversion_principle), which can be thought of as applying the adapter pattern, when the high-level class defines its own (adapter) interface to the low-level module (implemented by an adaptee class).
- [Ports and adapters architecture](./Ports_and_adapters_architecture)
- [Shim](./Shim_(computing))
- [Wrapper function](./Wrapper_function)
- [Wrapper library](./Wrapper_library)
...
1. [1](./Adapter_pattern#cite_ref-HeadFirst_1-0) [2](./Adapter_pattern#cite_ref-HeadFirst_1-1) [3](./Adapter_pattern#cite_ref-HeadFirst_1-2) Freeman, Eric; Freeman, Elisabeth; [Sierra, Kathy](./Kathy_Sierra); Bates, Bert (2004). [*Head First Design Patterns*](https://web.archive.org/web/20130504183857/http://www.headfirstlabs.com/books/hfdp/). [O'Reilly Media](./O'Reilly_Media). p. 244. [ISBN](./ISBN_(identifier)) [978-0-596-00712-6](./Special:BookSources/978-0-596-00712-6). [OCLC](./OCLC_(identifier)) [809772256](https://search.worldcat.org/oclc/809772256). Archived from [the original](http://www.headfirstlabs.com/books/hfdp/) (paperback) on 2013-05-04. Retrieved 2013-04-30.
2. [↑](./Adapter_pattern#cite_ref-GoF_2-0) Gamma, Erich; Helm, Richard; Johnson, Ralph; Vlissides, John (1994). [*Design Patterns: Elements of Reusable Object-Oriented Software*](https://archive.org/details/designpatternsel00gamm/page/139). [Addison Wesley](./Addison_Wesley). pp. [139ff](https://archive.org/details/designpatternsel00gamm/page/139). [ISBN](./ISBN_(identifier)) [0-201-63361-2](./Special:BookSources/0-201-63361-2).
3. [↑](./Adapter_pattern#cite_ref-Adapter_Design_Pattern_3-0) ["The Adapter design pattern - Problem, Solution, and Applicability"](https://web.archive.org/web/20170828230927/http://w3sdesign.com/?gr=s01&ugr=proble). *w3sDesign.com*. Archived from [the original](http://w3sdesign.com/?gr=s01&ugr=proble) on 2017-08-28. Retrieved 2017-08-12.
4. [↑](./Adapter_pattern#cite_ref-Structure_4-0) ["The Adapter design pattern - Structure and Collaboration"](https://web.archive.org/web/20170828231911/http://w3sdesign.com/?gr=s01&ugr=struct). *w3sDesign.com*. Archived from [the original](http://w3sdesign.com/?gr=s01&ugr=struct) on 2017-08-28. Retrieved 2017-08-12.

   [![Wikimedia Commons logo](//upload.wikimedia.org/wikipedia/en/thumb/4/4a/Commons-logo.svg/40px-Commons-logo.svg.png)](./File:Commons-logo.svg) Wikimedia Commons has media related to [Adapter pattern](https://commons.wikimedia.org/wiki/Category:Adapter%20pattern).    [![Wikibooks logo](//upload.wikimedia.org/wikipedia/commons/thumb/f/fa/Wikibooks-logo.svg/40px-Wikibooks-logo.svg.png)](./File:Wikibooks-logo.svg) The Wikibook *[Computer Science Design Patterns](https://en.wikibooks.org/wiki/Computer%20Science%20Design%20Patterns)* has a page on the topic of: ***[Adapter implementations in various languages](https://en.wikibooks.org/wiki/Computer%20Science%20Design%20Patterns/Adapter)***  
