...
```
The belief database explicitly identifies the circle-ellipse problem as a canonical violation of the Liskov Substitution Principle. This violation occurs with mutable objects where a Circle cannot truly substitute for an Ellipse (or vice versa) because operations that are valid on an ellipse (like setting different major and minor axes) would violate the invariants of a circle (where both axes must be equal). This makes the subclass instance non-substitutable wherever the superclass is expected, which is the core violation of LSP.
```
