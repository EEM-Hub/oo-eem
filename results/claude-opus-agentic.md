...
- **Q5**: CORRECT — Which SOLID principle states "A module should be responsible to one, and only one, actor"?
...
- **Q33**: CORRECT — In the Observer pattern, what distinguishes it from Publish-Subscribe?
...
```
The belief database explicitly states that the Liskov Substitution Principle 'is violated by the circle-ellipse problem with mutable objects.' While the rectangle-square problem is a closely related and also commonly cited example (a square is a special case of a rectangle, just as a circle is a special case of an ellipse), the canonical name for this LSP violation is the 'circle-ellipse problem' (also known as the square-rectangle problem). Since the belief database specifically names 'circle-ellipse' as the canonical violation, and that matches option (a), the answer is Circle-Ellipse.
```


## By Objective

- **SOLID Principles**: 5/6 (83%)
- **Core OO Principles**: 6/6 (100%)
- **Creational Patterns**: 5/5 (100%)
- **Structural Patterns**: 8/8 (100%)
- **Behavioral Patterns**: 12/12 (100%)
